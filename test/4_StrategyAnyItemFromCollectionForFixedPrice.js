...
describe("Strategy Any Item From Collection For Fixed Price", function () {
  // Deploy contract
  let strategyAnyItemFromCollectionForFixedPrice, owner, account1;
  ...
  before(async () => {
    ...
    strategyAnyItemFromCollectionForFixedPrice =
      await StrategyAnyItemFromCollectionForFixedPrice.deploy(0);
    await strategyAnyItemFromCollectionForFixedPrice.deployed();
  });

  it("Should fail: taker bid: always(maker ask isnt possible for this strategy)", async function () {
    const { makerAsk, takerBid } = await createMatchingMakerAndTakerOrder(true);

    let match = await strategyAnyItemFromCollectionForFixedPrice.canExecuteTakerBid(
      takerBid,
      makerAsk
    );
  ...
  it("Should pass: taker ask: matching taker ask and maker bid", async function () {
    ...
    let match = await strategyAnyItemFromCollectionForFixedPrice.canExecuteTakerAsk(
      takerAsk,
      makerBid
    );
  ...
  it("Should fail: taker ask: expired maker bid", async function () {
    ...
    match = await strategyAnyItemFromCollectionForFixedPrice.canExecuteTakerAsk(
      takerAsk,
      makerBid
    );
  ...
  it("Should fail: taker ask: non-matching price with maker bid", async function () {
    const { makerBid, takerAsk } = await createMatchingMakerAndTakerOrder(false);

    //Create mismatching price
    takerAsk.price = ethers.utils.parseEther("1");
